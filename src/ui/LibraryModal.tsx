    ...
    <Modal title="Containment Library" onClose={onClose} wide>
      ...
      {tab === 'enemies' && (
        <div className="grid gap-3 sm:grid-cols-2">
          {ENEMIES.map((enemy) => (
            <article key={enemy.id} className="rounded-lg border border-[#3f2a22] bg-[#1a1010] p-3">
      ...
      {tab === 'arsenal' && (
        <div className="grid gap-3 sm:grid-cols-2">
          {WEAPONS.map((weapon) => (
            <article key={weapon.id} className="rounded-lg border border-[#3f2a22] bg-[#1a1010] p-3">
              <div className="flex items-baseline justify-between gap-2">
          ...
          {SPECIALS.map((special) => (
            <article key={special.id} className="rounded-lg border border-[#5b2e24] bg-[#201010] p-3">
      ...
      {tab === 'rooms' && (
        <div className="grid gap-3 sm:grid-cols-2">
          {MAPS.map((room) => (
            <article key={room.id} className="rounded-lg border border-[#3f2a22] bg-[#1a1010] p-3">
